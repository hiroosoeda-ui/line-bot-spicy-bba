...
SYSTEM_PROMPT_FILE  = os.environ.get("SYSTEM_PROMPT_FILE", "systemprompt.txt")   # 相対
SYSTEM_PROMPT_PARAM = os.environ.get("SYSTEM_PROMPT_PARAM")  # 例: /ronna
...
def _load_system_prompt() -> str:
    ...
    try:
        ...
        path = SYSTEM_PROMPT_FILE if os.path.isabs(SYSTEM_PROMPT_FILE) else os.path.join(base, SYSTEM_PROMPT_FILE)
        with open(path, "r", encoding="utf-8") as f:
            p = f.read().strip()
            if p:
                logger.info(f"Loaded system prompt from file: {path}")
                return p
    except Exception as e:
        logger.warning(f"Prompt file not used: {e}")

    # 2) SSMパラメータから読む
    if SYSTEM_PROMPT_PARAM:
        try:
            v = _ssm.get_parameter(Name=SYSTEM_PROMPT_PARAM, WithDecryption=True)["Parameter"]["Value"].strip()
            if v:
                logger.info(f"Loaded system prompt from SSM: {SYSTEM_PROMPT_PARAM}")
                return v
        except Exception as e:
            logger.warning(f"Prompt SSM not used: {e}")

    # 3) フォールバック（埋め込み）
    logger.info("Using embedded default system prompt")
    return (
        "あなたは『スパイシー☆ババア』。ルー大柴風に英語をMIXして、スパイシーでファンキーな関西弁でしゃべる。"
    )
